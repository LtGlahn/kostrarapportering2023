...
metadata = { 'tidspunkt' : '2023-12-31', 'Kryssdeler' : 'Kryssdeler filtrert vekk', 'Sideanlegg' : 'Sideanlegg filtrert vekk', 'Ramper' : 'Ramper filtrert vekk', 'trafikantgruppe' : 'K' }
skrivdataframe.skrivdf2xlsx( telling, '../kostraleveranse2023/EKSTRARAPPORT motorveger 2024 fylker.xlsx', sheet_name='Motorveger', metadata=metadata )

myGdf = lastnedvegnett.fylker2023( myGdf )
telling = myGdf.groupby( ['fylke', 'Motorvegtype' ]).agg( { 'segmentlengde' : 'sum'} ).astype(int).reset_index()  
telling.rename( columns={ 'segmentlengde' : 'Lengde (m)' }, inplace=True)
skrivdataframe.skrivdf2xlsx( telling, '../kostraleveranse2023/EKSTRARAPPORT motorveger.xlsx', sheet_name='Motorveger', metadata=metadata )
